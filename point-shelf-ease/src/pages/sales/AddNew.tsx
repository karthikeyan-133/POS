...
import { Search, Scan, Plus, X, Tag } from 'lucide-react';
...
export default function AddNewSalePage() {
  ...
  const handleConvertFromQuotation = () => {
    navigate('/quotations?select=true');
  };
    ...
    <div className="space-y-8 p-1 bg-gradient-subtle min-h-screen">
      <div className="rounded-xl p-6 border-2 bg-white border-border shadow-lg">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-4xl font-bold tracking-tight mb-2 text-foreground bg-gradient-hero bg-clip-text text-transparent">New Sales Order</h1>
            <p className="text-lg text-muted-foreground">Create a new sales order</p>
          </div>
          <Button 
            onClick={handleConvertFromQuotation} 
            variant="gradient"
      ...
      <Card className="hover:shadow-xl transition-all duration-300 border border-border bg-gradient-to-br from-vibrant-green/5 to-vibrant-teal/5">
        <CardHeader className="bg-gradient-primary rounded-t-lg">
          ...
          <CardDescription className="text-base text-white/90">Fill in the sales order information</CardDescription>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="so_number" className="text-foreground">SO Number</Label>
                <Input 
                  id="so_number" 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Select value={form.customer_name} onValueChange={(value) => handleChange('customer_name', value)}>
                  ...
                  <SelectContent>
                    {customers.map(customer => (
                      <SelectItem key={customer.id} value={customer.name} className="hover:bg-vibrant-green/10">
                        {customer.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select value={form.store_location} onValueChange={(value) => handleChange('store_location', value)}>
                  ...
                  <SelectContent>
                    {stores.map(store => (
                      <SelectItem key={store.id} value={store.name} className="hover:bg-vibrant-green/10">
                        {store.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                <Select value={form.payment_status} onValueChange={(value) => handleChange('payment_status', value)}>
                  ...
                  <SelectContent>
                    {paymentStatuses.map(status => (
                      <SelectItem key={status.value} value={status.value} className="hover:bg-vibrant-green/10">
                        {status.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <div className="border-t border-vibrant-green/20 pt-6">
              <h3 className="text-lg font-semibold mb-4 text-vibrant-green">Product Search & Barcode Scanning</h3>
              <div className="flex gap-4 mb-4">
                ...
                <Button 
                  ...
                  variant="outline"
                  className="border-vibrant-green/30 text-vibrant-green hover:bg-vibrant-green/10"
                >
              ...
              {showBarcodeScanner && (
                <div className="bg-vibrant-green/10 p-4 rounded-lg mb-4 border border-vibrant-green/20">
                  <div className="text-center">
                    <Scan className="h-12 w-12 mx-auto mb-2 text-vibrant-green" />
                    <p className="text-vibrant-green mb-2 font-medium">Barcode Scanner Active</p>
              ...
              {scannedItems.length > 0 && (
                <div className="border border-vibrant-green/20 rounded-lg p-4 bg-white">
                  <h4 className="font-semibold mb-3 text-vibrant-green">Scanned Items</h4>
                  <div className="space-y-2">
                    {scannedItems.map(item => (
                      <div key={item.id} className="flex items-center justify-between bg-vibrant-green/5 p-3 rounded border border-vibrant-green/10">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-vibrant-green">${item.price.toFixed(2)}</span>
                          <Button
                            type="button"
                            size="sm"
                            variant="outline"
