...
const SAMPLE: SalesReturn[] = [
  {
    id: 'sru1',
    lastModifiedAt: '2025-02-15 14:30:00',
    orderNumber: 'SRN-2001',
  ...
  {
    id: 'sru3',
    lastModifiedAt: '2025-02-17 11:45:00',
    orderNumber: 'SRN-2003',
    paymentStatus: 'Pending Refund',
    storeLocation: 'Outlet Store',
...
export default function SalesReturnListPage() {
  ...
  const [items] = useState<SalesReturn[]>(SAMPLE);
  ...
  return (
    <Card className="hover:shadow-lg transition-all duration-300 border border-border bg-white">
      <CardHeader className="bg-slate-50 rounded-t-lg">
        <CardTitle className="text-xl font-bold text-foreground">Sales Returns</CardTitle>
        <CardDescription>All sales return entries</CardDescription>
        <div className="relative">
          <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
          <Input placeholder="Search by order number, customer, store, or payment status..." value={query} onChange={(e) => setQuery(e.target.value)} className="pl-10" />
        </div>
      </CardHeader>
              ...
              <TableRow key={i.id}>
                ...
                <TableCell>
                  <span className={`px-2 py-1 rounded-full text-xs ${
                    i.paymentStatus === 'Refunded' ? 'bg-green-100 text-green-800' :
                    i.paymentStatus === 'Partial Refund' ? 'bg-yellow-100 text-yellow-800' :
                    i.paymentStatus === 'Pending Refund' ? 'bg-blue-100 text-blue-800' :
