...
import { Download, TrendingUp, DollarSign, CreditCard, Calendar, Users } from 'lucide-react';
...
export default function CustomerPaymentReport() {
  ...
  useEffect(() => {
    fetchCustomerPaymentReports();
  }, [dateRange]);

  const fetchCustomerPaymentReports = async () => {
    try {
      setLoading(true);
      ...
      const customerStats = payments.reduce((acc: Record<string, { total_sales: number; total_paid: number; payment_count: number; last_payment: string }>, payment) => {
        const customerName = payment.sales?.customers?.name || 'Walk-in Customer';
        if (!acc[customerName]) {
          acc[customerName] = { total_sales: 0, total_paid: 0, payment_count: 0, last_payment: '' };
        }
        ...
        if (paymentDate > acc[customerName].last_payment) {
          acc[customerName].last_payment = paymentDate;
        }
      ...
      console.error('Error fetching customer payment reports:', error);
      toast({
        title: 'Error',
        description: 'Failed to fetch customer payment report data',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const exportReport = () => {
    toast({
      title: 'Export',
      description: 'Customer payment report exported successfully',
    });
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg">Loading customer payment reports...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Customer Payment Report</h1>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Pending amounts by customer</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={customerData.filter(c => c.pending_amount > 0).slice(0, 10)}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="customer" angle={-45} textAnchor="end" height={100} />
                <YAxis />
                <Tooltip formatter={(value) => formatCurrency(Number(value))} />
