...
export default function PurchasePaymentReport() {
  ...
  const fetchPaymentReports = async () => {
    try {
      ...
      const supplierStats = payments.reduce((acc: Record<string, { total_purchases: number; total_paid: number; payment_count: number }>, payment) => {
        const supplierName = payment.purchases?.suppliers?.name || 'Unknown Supplier';
        if (!acc[supplierName]) {
          acc[supplierName] = { total_purchases: 0, total_paid: 0, payment_count: 0 };
        }
  ...
  const exportReport = () => {
    toast({
      title: 'Export',
      description: 'Purchase payment report exported successfully',
    });
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Track purchase payments and outstanding amounts</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Pending Payments by Supplier</CardTitle>
            <CardDescription>Outstanding amounts by supplier</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={supplierData.filter(s => s.pending_amount > 0)}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="supplier" angle={-45} textAnchor="end" height={100} />
                <YAxis />
                <Tooltip formatter={(value) => formatCurrency(Number(value))} />
